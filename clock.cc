...
static int usage(const char *progname) {
  ...
  fprintf(stderr,
          ...
          "\t-C <r,g,b>        : Time color. Default 255,255,0\n"
          "\t-B <r,g,b>        : Background-Color. Default 0,0,0\n"
          "\t-O <r,g,b>        : Outline-Color, e.g. to increase contrast.\n"
          "\t-c <r,g,b>        : Date color. Default 255,255,0\n"
          "\t-D <date-format>  : Default '%D'. See strftime()\n"
          );
...
int main(int argc, char *argv[]) {
  ...
  const char *date_format = "%D";
  Color color(255, 255, 0);
  Color dcolor(255, 255, 0);
  ...
  int opt;
  while ((opt = getopt(argc, argv, "x:y:f:C:B:O:b:S:d:c:D:")) != -1) {
    switch (opt) {
    case 'd': time_format = strdup(optarg); break;
    case 'D': date_format = strdup(optarg); break;
  ...
  while (!interrupt_received) {
      ...
      rgb_matrix::DrawText(offscreen, font, x, y + font.baseline(),
                           ...
                           letter_spacing);
      rgb_matrix::DrawText(offscreen, font, 0, 14 + font.baseline(),
                           dcolor, NULL, date_buffer,
                           letter_spacing);
